...
class DayQueryType extends QueryType
{
    protected function generateQuery(string $modelClass, TotalRecordsDto $totalRecordsDto, string $seriesSql): Builder
    {
        $query = $modelClass::selectRaw(
            'DATE(' . $totalRecordsDto->colXaxis . ') AS cat,
            DATE(' . $totalRecordsDto->colXaxis . ') AS catorder,
        ...
        if (is_numeric($totalRecordsDto->advanceFilterSelected)) {
            ...
            $query->where($totalRecordsDto->colXaxis, '>=', now()->subDay($totalRecordsDto->latestData + 1));
        }
